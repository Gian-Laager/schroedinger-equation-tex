...
An extension to the function in snippet~\ref{code:newtons-method} is implemented in the struct \rustinline{NewtonsMethodFindNewZero} which can be used to find multiple roots.
...
\begin{rustcode}[caption = {
    ...
    and Such that functions with states, like wave functions that store parameters, can be integrated
...
calculated. It can be interpreted such that the oscillating part of the wave function has to complete all half oscillation. \cite{hall2013quantum} also states that the equations from
section~\ref{meth:wkb:approximation-scheme} have to agree, up to a multiplication by a constant which will be the case iff equation~\ref{eq:energy-cond} is satisfied.
...
Because~\ref{eq:prog_cond} is not continuous, Newtons method can't be applied. Further on the bounds of integration have to be finite, this means the user of the program will have to
...
\begin{rustcode}[caption={
    The implementation checks for discontinuities in the first the interval \rustinline{(0.0, ENERGY_STEP)} is checked, if there are not enough zeros the next interval is checked. This is repeated until $n$ zeros have been found.
...
\subsection{Accuracy}
For a benchmark the values below, will be used.
\begin{align*}
  m = 1 \\
  V(x) = x^{2} \\
  (-\infty, \infty) \approx (-200, 200).
\end{align*}
To get the actual values the Wolfram Language with \emph{WolframScript} was used. \emph{Wolfram Language} is the programing language behind \emph{Mathematica}\footnote{State of the art technical computing program~\citep{wolfram2022description}}, it can calculate the integral analytically and precisely.
...
\end{align}
Further on it can assumed that the Airy function is only valid in a closed interval, this means that there must be at least two roots of equation~\ref{eq:valid-airy}. These roots will be called turning point boundaries from now on. The factor of $a$ is used to emulate the behavior of $\ll$.

The left boundary point must have a positive and the right a negative derivative. This means  after calculating the roots, they can be group together by there derivatives.
\\
In order to find all roots a modified version of Newtons method. When we find a solution, $x_{0}$ we can divide the original function by $(x - x_{0})$ this means that Newtons method wont be able to find $x_{0}$ again.
\\
\label{sec:view}
...
Newtons method works well if the value of $f(x)$ is small and $f'(x)$ is neither to small nor to big.
As a rating function
\[
  \label{eq:rating-newton}
  \sigma(x) = \cfrac{|f(x)| }{ -\exp\left({{\left(\deriv{f}{x}(x)\right)}^{2} + 1}\right)}
\]
is used, where lower is better. This function is just an educated guess, but it has to have some properties, as the derivative of $f$ tends to 0, $\sigma(x)$ should diverge to infinity.
...
\\
After we rated all of the 1000 guesses we can pick the best one as a first guess and use the modified Newtons method with it. We do this process 256 times by default. In theory we could therefore use the WKB approximation for potentials with up to 256 turning points.
...
point is on the left and negative when its on the right side of the turning point.
...
The second is Density functional field theory, it would be interesting to add to the program in the future, but it would require major changes in the architecture.
\\
...
But there functions did not join nicely in the middle of the two turning points. To maintain smoothness only one turning point was there for used. This issue will be discussed in
...
\begin{figure}[ht]
  ...
  \caption{\label{fig:absolute-error-with-wave} The absolute from figure~\ref{fig:absolute-error} is plotted with the approximate wave function. As the violet line indicates the error reaches a local maxima
