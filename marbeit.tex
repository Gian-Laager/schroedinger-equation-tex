...
\subsection{Usage}
First you need to install GNU Plot.
For Linux the package should be in your distros repository with the name \emph{gnuplot}

On Windows you can download the installer on \url{http://gnuplot.info/}. There should be a
box with the title \emph{Version [version] (current)} at the time of writing this document
the version is 5.4.5. Then click on the link \emph{Release [version] ([date])}. Then you
should be redirected to SOURCEFORGE where you can download the \emph{gp545-win64-mingw.exe}
file (the 545 might be a different number). Then go through the installer. In the \emph{Select Additional Tasks} window change \emph{Add application directory to your PATH environment variable} to true.

After the installation is finished you can open a terminal (CMD on Windows) and go to the
directory where you've downloaded the code for \bashinline{shroedinger_approx}. You then need
to run the program according to chapter~\ref{sec:manual}
...
\subsection{Windows}
\subsubsection{Rust}
Download \emph{RUSTUP-INIT.EXE (64-BIT)} from \\
\hspace*{3ex}\url{https://www.rust-lang.org/tools/install} \\
and run the installer.
\\[3ex]
You'll be prompted to choose between \emph{Quick install}, \emph{Manually install} and
\emph{Dont't install}. Type \emph{1} and enter to choose the first option. This will download
Visual Studio a IDE from Microsoft, go through the installation with the default options.
Once you've installed Visual Studio the installer will prompt you to confirm the installation options. Type \emph{1} and enter

\subsubsection{Go}
Because \emph{shroedinger\_approx} uses a Go library to calculate the Airy functions, you'll
also need to install the Go compiler.
\\[3ex]
Download the installer on\\
\hspace*{3ex}\url{https://go.dev/doc/install}\\
...
\subsubsection{shroedinger\_approx}
Go to \\
\hspace*{3ex}\url{https://github.com/Gian-Laager/Schroedinger-Approximation}\\
and click the green button that says \emph{Code}.
If you aren't familiar with \emph{git} you can download the code as a ZIP file.
