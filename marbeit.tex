...
Rust as of the time of writing this document is not yet standardized, meaning the code provided might no longer be correct with one of the newer Rust versions. The program was written for version \emph{1.67.0-nightly}.
...
\end{align*}
where $\mi$ is the imaginary unit,
...
\\
All equations from now on will be in terms of Planck units and
...
Most of the metrics produced \emph{will not} be used. An example reproduced of a benchmark of \emph{schroedinger\_approx} is reproduced on the next page.
...
\begin{figure}[H]
	...
	\caption{Illustration of Newton's method, $f(x) = (x-2)(x-1)(x+1)$.The blue line
      indicates the initial guess which in this case is $2.5$. The $g(x)$ is a tangent to $f(x)$ at
...
The struct \rustinline{NewtonsMethodFindNewZero} is an extension to the function in snippet~\ref{code:newtons-method}, it can be used to find multiple roots.
...
Using the formula from~\cite[section 15.6.4]{hall2013quantum}
\begin{align*}
    \delta = 0.5 \\
...
\[
  h(x) =  \left\{
    \begin{array}{ll}
      f(x) & x < \alpha \\
      g(x) & x > \alpha + \delta \\
      f(x) + (g(x) - f(x))\chi(\cfrac{x-\alpha}{\delta}) & \text{else}
    \end{array}
    \right.
\]
...
\chapter{Calculation}
This chapter discusses the calculations in the program utilizing the methods described in chapter~\ref{chap:methods}.
...
A point $x$ where $V(x) = E$ is called a \emph{turning point}. It is assumed that the WKB function is a good approximation in the region where
\begin{align}
  \label{eq:valid}
...
\end{align}
Further on it can be assumed that the Airy function is only valid in a closed interval, which means that there must be at least two roots of equation~\ref{eq:valid-airy}. These roots will be called \emph{turning point boundaries} from now on. The factor of $a$ is used to emulate the behavior of $\ll$.
...
\\
In order to find all roots a modified version of Newton's method is used. When we find a solution $x_{0}$ we can divide the original function by $(x - x_{0})$. This means that Newton's method won't be able to find $x_{0}$ again.
\\
\label{sec:view}
To later plot the wave function we will define the so-called ``view''. This is the interval which the user gets to see in the end.
It is defined to be
\begin{align*}
  t_{l} < t_{r} \\
...
\end{rustcode}
Here \rustinline{make_guess} uses $\sigma(x)$, and returns the best guess. \rustinline{NewtonsMethodFindNewZero} is the modified version of Newton's method where all the roots are stored and its
...
guess of the turning point. We will do this for both the left- and rightmost turning point
...
\begin{description}
    ...
    \item[$\operatorname{Ai}()$] First variant of the Airy functions~\citep{math:airy}. It arises from solving the Schrödinger equation analytically at the turning points~\cite[section
            15.5]{hall2013quantum}. Note from here on out this function will be referred to as \emph{the Airy function}.
    \item[$\psi^{Airy}$] Airy function part around the turning points.
\end{description}
...
\begin{figure}[H]
  ...
  \caption{Left half of wave function with $N_{Energy} = 9 \Rightarrow E \approx 9.49$, $m = 2$, $V(x) = x^{2}$}\label{fig:wave_parts}
\end{figure}
...
\end{align*}
as~\cite{zwiebach2018lecture} showed in his lecture. From the formula he derives it can be stated that WKB is valid if and only if
\[
...
All the equations of the WKB approximation are split into multiple parts. This is also reflected in the program architecture.
...
Two joints are constructed and they have the highest \emph{priority} when evaluating an ApproxPart for a given $x$.
...
\end{rustcode}
The term \emph{priority} is used to say how far up the if statement the function is. Or in other words, functions with a higher priority are preferred. Because the joints overlap with both the ranges of the Airy and WKB function is important that they are given a higher priority. Further on the range of the Airy part is also included in the WKB range
because of this the WKB part has the least priority.

As we can see, the check if the joints are even enabled happens here, because \newline \rustinline{ENABLE_AIRY_JOINTS} is a constant. The compiler will
...
Once we have all the \rustinline{WkbWaveFunction} instances they need to group with the \rustinline{AiryWaveFunction} instances. Using those pairs we can finally construct all the
...
You can reproduce all the results yourself according to the manual in appendix~\ref{sec:manual}.
...
\begin{figure}[H]
  ...
  \caption{Wave function of 39th energy with $V(x) = x^{2}$, $m = 1.$}
\end{figure}
...
\]
In theory this is possible (proof appendix~\ref{proof:zero-wave-function}) but can't be physically valid because the Schrödinger equation
...
impact on the user interface which is probably the weakest part of the program. It is only accessible to programmers and can't be used to it's full potential if the user doesn't know Rust, a systems
...
\\[3ex]
Shortly before I had to finish this paper I realized that there was an interesting corresponds between the Dirac formalism~\citep{xylyxylyx2021dirac} and the architecture of the program. This happened by
accident and the implementation could be slightly changed such that \rustinline{Superposition} structs represent vectors in Hilbert space. In this case \rustinline{WaveFunction} structs would act as a
set of bases vectors. If one would manage to proof that the states produced from different energies of \rustinline{WaveFunction} are orthogonal in Hilbert space, the Dirac formalism could in theory be
used to its full extend. The problem with this idea is what the dual vector space to the superpositions should be and how it should be represented. The dual space can be constructed with the inner
product, but the question remains on how to define an inner product on two wave functions.
\\[3ex]
...
\\[3ex]
First you'll need to install the package manager \emph{Homebrew}, which can be used to install software without downloading all the installers manually. To install it open the \emph{Terminal} program, paste the command below and press enter.
\begin{bashcode}
...
\\[3ex]
After we've installed Homebrew, we can install the dependencies of \emph{schroedinger\_approx} with the command
\begin{bashcode}
...
\\[3ex]
After we've installed all the dependencies you can go to \url{https://github.com/Gian-Laager/Schroedinger-Approximation} and click the green \emph{Code} button,
...
follow the instructions of the terminal. Once you're logged in run the commands
\begin{bashcode}
sudo apt-get update
...
Click the green \emph{Code} button and download the code as a ZIP file and extract it to a directory of your choice.
...
\end{bashcode}
You then need to replace all the \bashinline{\} to \bashinline{/} and change \bashinline{C:} to \bashinline{/mnt/c} (if you have another letter use it's lowercase at \bashinline{/mnt/<your letter>}) and
add a \bashinline{cd} at the front. For the example above it would look like this:
\begin{bashcode}
...
You have to replace all the comments (\rustinline{/*...*/}) with the values you want.
...
\\
And as for the scaling type, choose one of the options described at the end of section~\ref{sec:scaling-type}.
...
To create a custom potential you'll have to define a function as shown below.
\begin{rustcode}
fn my_potential(x: f64) -> f64 {
    return /*some calculation*/;
}
\end{rustcode}
\rustinline{my_potential} a name that you can choose and have to use later when you're passing it to \rustinline{WaveFunction::new}.
\rustinline{/*some calculation*/} can be any Rust code that results in a \rustinline{f64}.
\\
\subsubsection{Examples}
Hyperbolic cosine
\begin{rustcode}
fn hyperbolic_cos(x: f64) -> f64 {
    return x.cosh();
}
\end{rustcode}
...
To check if branches with a constant condition of \rustinline{false} get removed we will use ``Compiler Explorer'' on \url{https://godbolt.org/}.
