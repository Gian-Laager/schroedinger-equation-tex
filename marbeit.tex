...
Because~\ref{eq:prog_cond} is not continuous, Newton's method can't be applied. Further on the bounds of integration have to be finite, this means the user of the program will have to
...
\\
In order to find all roots a modified version of Newton's method. When we find a solution, $x_{0}$ we can divide the original function by $(x - x_{0})$ this means that Newton's method wont be able to find $x_{0}$ again.
\\
\label{sec:view}
...
Newton's method works well if the value of $f(x)$ is small and $f'(x)$ is neither to small nor to big.
...
From experience the results are inaccurate but still usable. At least in the case where the turning points were missed by Newton's method the WKB parts were fairly accurate, but unsurprisingly diverged at
...
Because the superposition principal is also applicable to energies it is possible that
