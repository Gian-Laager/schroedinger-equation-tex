...
\begin{center}
\begin{math}
  \mi\hbar {\frac {\partial }{\partial t}}\Psi (x,t)=\left[-{\frac {\hbar ^{2}}{2m}}{\frac {\partial ^{2}}{\partial x^{2}}}+V(x,t)\right]\Psi (x,t)
\end{math}
\end{center}
Or more general
\begin{center}
\begin{math}
  \mi\hbar {\frac {\partial }{\partial t}}\Psi (x,t)=\hat{H} \Psi(x,t)
\end{math}
\end{center}
...
\begin{align*}
l_{\text{Planck}} = l_{\text{SI}} \sqrt{\cfrac{G \hbar}{c^{3}}}  && \sqrt{\cfrac{G \hbar}{c^{3}}}  \approx 1.616255(18) \cdot 10^{-35}~\text{m}   \\
m_{\text{Planck}} = m_{\text{SI}} \sqrt{\cfrac{\hbar c}{G}}      && \sqrt{\cfrac{\hbar c}{G}}      \approx  2.176434(24) \cdot 10^{-8}~\text{kg} \\
t_{\text{Planck}} = t_{\text{SI}} \sqrt{\cfrac{\hbar G }{c^{5}}} && \sqrt{\cfrac{\hbar G }{c^{5}}} \approx 5.391247(60) \cdot 10^{-44}~\text{s}
\end{align*}
...
\[
  h(x) =  \left\{
    \begin{array}{ll}
      f(x) & x < \alpha \\
      g(x) & x > \alpha + \delta \\
      f(x) + (g(x) - f(x))\chi(\frac{x-\alpha}{\delta}) & else
    \end{array}
    \right.
\]
...
\begin{align}
  ...
  \frac{1}{2\pi}\int_{-\infty}^{\infty}p(x) dx - \frac{1}{2} \mod{1} = 0
\end{align}
...
WolframScript. As a ruff estimate we would expect an error of $\pm \frac{10}{64000} \approx \pm 1.56 * 10^{-4}$, because the program checks for energies with that step size.
...
\label{eq:wkb:t}
  V(t) - E = 0 \\
\label{eq:wkb:exp}
  \psi^{WKB}_{exp}(x)= e^{\delta\mi} {c_{1} \over 2 \sqrt{p(x)}} \exp\left(-\left|\int_{x}^{t}p(y) dy\right|\right) \\
\label{eq:wkb:osz}
  \psi^{WKB}_{osz} (x)= {c_{1} \over \sqrt{p(x)}} \exp\left(-\left|\int_{x}^{t}p(y) dy\right|\mi + \delta \mi\right) \\
  u_{1} = -2 m \frac{dV}{dx}(t) \\
\label{eq:airy}
  \psi^{Airy}(x) = e^{(t - x + \delta)\mi} {c_{1} \sqrt{\pi} \over \sqrt[6]{u_{1}}} \operatorname{Ai}\left(\sqrt[3]{u_{1}} (t - x)\right).
\end{align}
...
\begin{align}
  \label{eq:valid}
  -\frac{1}{2m} \deriv{V}{x}(x) \ll (V(x) - E)^{2}.
\end{align}
...
\begin{align}
  \label{eq:valid-airy}
  -\frac{1}{2m} \deriv{V}{x}(x) - (V(x) - E)^{2} > 0
\end{align}
...
\[
  ...
  \sigma(x) = {|f(x)| \over -\exp\left({{\left(\deriv{f}{x}(x)\right)}^{2} + 1}\right)}
\]
...
\[
  \sigma(x) = \left\{
    \begin{array}{ll}
    {|f(x)| \over -\exp\left({{\left(\deriv{f}{x}(x)\right)}^{2} + 1}\right)} & f(x) \ne 0~\text{and}~\deriv{f}{x} \ne 0 \\
    0 & \text{else}
    \end{array}\right.
\]
...
\begin{align}
  \chi(x) = \sin^{2}\left(\frac{\pi(x - \alpha)}{2\delta}\right) \\
  (f \sqcup g)(x) = f(x) + (g(x) - f(x)) \chi(x)
\end{align}
and proof that
\begin{align}
  \label{eq:deriv-f}
  \frac{d(f \sqcup g)}{dx}(\alpha) = \frac{df}{dx}(\alpha) \\
  \label{eq:deriv-g}
  \frac{d(f \sqcup g)}{dx}(\alpha + \delta) = \frac{dg}{dx}(\alpha + \delta).
\end{align}
Calculate derivatives
\begin{align}
  \deriv{\chi}{x}(x) = \frac{\pi}{2 \delta} \sin\left(\frac{\pi (x - \alpha)}{\delta}\right)\\
