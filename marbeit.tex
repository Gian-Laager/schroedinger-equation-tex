...
\begin{figure}[H]\label{fig:color-to-phase-circle}
  \centering
  \includegraphics[width=0.8\textwidth]{plots/color_circle.png}
  \caption{Illustration of the mapping of an angle to color to represent complex numbers}
\end{figure}
...
This section is a guide on how to install \emph{shroedinger\_approx} on your computer. Please follow the guide for your operating system.
\subsection{Linux}
...
\begin{figure}[H]\label{fig:wave_parts}
  ...
  \caption{Left half of wave function with $N_{Energy} = 9 \Rightarrow E \approx 13.4$, $m = 2$, $V(x) = x^{2}$}
\end{figure}
...
\label{sec:osc_turnging_point_problem}
...
\begin{minipage}{\textwidth}
If the energy is high enough, the two oscillations will eventually merge into one as shown in figure~\ref{fig:mexican-hat-high-energy}.
\begin{figure}[H]\label{fig:mexican-hat-high-energy}
  \centering
...
\begin{minipage}{\textwidth}
  Unfortunately the program is not able to calculate the 55th energy because Newtons method fails to find the turnging points in the middle. And with 54th energy the program doesn't generate any errors
  but the region around $x = 0$ can't be correct (Figure~\ref{}).
\begin{figure}[H]\label{fig:mexican-hat-high-energy}
  \centering
  \includegraphics[width=\textwidth]{plots/mexican-hat-53.pdf}
  \caption{
    Wave function of mexican hat potential, with nth energy 55 and $m = 1$. This is the first energy where the oscillating parts combine. The middle of the function has a rather low frequency compared
    to the other oscillating parts.
  }
\end{figure}
\end{minipage}
