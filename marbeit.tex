...
We assume that the wave function, $\Psi(x)$ will diverge to 0 as $x$ goes to infinity.

The program uses Planck units which means $c = \hbar = G = 1$
...
\begin{align*}
  ...
  \compconj{z} = a - b \mi \\
  \|z\|^{2} = a^{2} + b^{2} \\
  e^{a+b\mi} = e^{a} \left( \cos(b) + \mi \sin(b) \right)
\end{align*}
...
The last equation is the Euler's formula, it rotates a number in the complex plane by $b$ radians.
